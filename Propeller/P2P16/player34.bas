...
for i=0 to 2559: poke scope_ptr+i,0: next i
'' --------------------------------- THE MAIN LOOP --------------------------------------------------------------------------------------
...
412 if (ansibuf(3)=$28 orelse ansibuf(3)=$58) andalso panel=1 then
    ...
    if filenum2>0 then get #7,1+78*(filenum2-1),displayname(0),78				' find the file name
    get #7,,displayname(0),78: filename$="" : for i=0 to 77
    if displayname(i)>=32 then filename$+=chr$(displayname(i))
     next i
    'input #7,filename$ 	:
     filename$=rtrim$(filename$)	
'       position 0,0: print filename$;"-------------------- "		
								' clean trailing spaces
    close #7
    let ext$=lcase$(right$(filename$,3))
    if ext$="mod" or ext$="dmp" or ext$="sid" or ext$="spc" or ext$="wav" or ext$="lac" or ext$="mp3" then    ' if the file is playable, stop playing the previous one
      ...
      if mp3cog>0 then cpustop(mp3cog) : stopaudio : mp3cog=-1      
      waveplaying=0: modplaying=0 : spcplaying=0 : sidplaying=0: dmpplaying=0 : spcplaying=0 :mp3playing=0   ' clear playing indicator variables
...
'    endif
  ...
  if ext$="mp3" then  										' this is a mp3 file. 
  ...
  if filemove>0 then											' move the "cursor" down
    if panel=0 then											' 0 - directory panel
      ...
      for ii=dirnum2-9 to dirnum2
       get #9,1+78*ii,displayname(0),78									' get the name
       j=78 : do : j-=1 : loop until displayname(j)>32:  k=j 	
       filename$="": for i=0 to k : filename$+=chr$(displayname(i)): next i 
       filename2$=rtrim$(filename$)
       if len(filename2$)>36 then filename2$="(...)"+right$(filename2$,33) : k=37
        	     			        ' find the end of the nam
        v.box(8,(ii+4-dirnum2+9)*16,352,(ii+4-dirnum2+9)*16+15,193)
        position ((360-8*k-8)/8, ii+4-dirnum2+9) : v.write(filename2$)
''        for j=0 to k: v.write(chr$(displayname(j))) :next j
    ...
    if panel=1 then 											' 1 - file panel
      ...
      for ii=filenum2-20 to filenum2
       get #9,1+78*ii,displayname(0),78									' get the name
        j=78 : do : j-=1 : loop until displayname(j)>32:  k=j 		     			        ' find the end of the nam

       filename$="": for i=0 to k : filename$+=chr$(displayname(i)): next i 
       filename2$=rtrim$(filename$)
       if len(filename2$)>33 then filename2$="(...)"+right$(filename2$,33): k=37

        v.box(368,(ii+4-filenum2+20)*16,712,(ii+4-filenum2+20)*16+15,34)
        position ((720+360-8*k-8)/8, ii+4-filenum2+20): v.write(filename2$)
   '     for j=0 to k: v.write(chr$(displayname(j))) :next j
      next ii
      close #9
      highlight(1,filenum1,1)                                      
    ...
    if panel=0 then											' dir panel
      ...
      for ii=dirnum2 to dirnum2+9
       ...
       if len(filename2$)>33 then filename2$="(...)"+right$(filename2$,33): k=37
        ...
        position ((360-8*k-8)/8, ii+4-dirnum2) : if ii<dirnum3 then v.write(filename2$)
  '      if ii<dirnum3 then for j=0 to k: v.write(chr$(displayname(j))) :next j
    ...
    if panel=1 then											' file panel, move up, the same stuff again
      ...
      for ii=filenum2 to filenum2+20
       get #9,1+78*ii,displayname(0),78									' get the name
        j=78 : do : j-=1 : loop until displayname(j)>32:  k=j 		     			        ' find the end of the name
       ...
       if len(filename2$)>33 then filename2$="(...)"+right$(filename2$,33): k=37
        ...
        position ((720+360-8*k-8)/8, ii+4-filenum2) : if ii<filenum3 then v.write(filename2$)
...
360 if e=4 then										' error #4=file not found - create a new file
  ...
  print #5,".."+space$(75)								' this file system has no .. - it needs to be manually added
  filename$ = dir$("*", fbDirectory)							' find all directories
  while filename$ <> "" andalso filename$ <> nil
    if len(filename$)<77 then filename$=filename$+space$(77-len(filename$))		' the string has to be exactly 38 characters 
    filename$=right$(filename$,77)							' which enables to use get when reading
...
if e=0 then 										' now the directory list exists
  ...
  i=2
  do
    line input #5,filename$									' write first 10 entries to the panel
    filename2$=rtrim$(filename$)
    if len(filename2$)>36 then filename2$="(...)"+right$(filename2$,33)
...
410 if e=4 then
  ...
  while filename$ <> "" andalso filename$ <> nil
    if len(filename$)<77 then filename$=filename$+space$(77-len(filename$))
    filename$=right$(filename$,77)
    print #5, filename$
    filename$ = dir$()
        if waveplaying orelse mp3playing then getwave
...
' try to find a proper header  

'//if header(0)=0 then
'//  let wavepos2=wavepos: do: wavepos2+=100000: get #8,wavepos,header(0),4 : loop until header(0)<>0: wavepos2-=100000
'//  do: wavepos2+=10000: get #8,wavepos,header(0),4 : loop until header(0)<>0: wavepos2-=10000
'//  do: wavepos2+=1000: get #8,wavepos,header(0),4 : loop until header(0)<>0: wavepos2-=1000
'//  do: wavepos2+=100: get #8,wavepos,header(0),4 : loop until header(0)<>0: wavepos2-=100
'//  do: wavepos2+=10: get #8,wavepos,header(0),4 : loop until header(0)<>0: wavepos2-=10
'//  wavepos=wavepos2
'//  endif

do until header(0)=$ff andalso ((header(1) and $F0)=$F0 orelse (header(1) and $F0)=$E0)
  get #8,wavepos,filebuf(0),4098
  for i=0 to 4095
      if filebuf(i)=$ff andalso ((filebuf(i+1) and $F0)=$F0 orelse (filebuf(i+1) and $F0)=$E0) then 'this is a header
         wavepos+=i
         goto 1940
         endif
       next i  
...
1900 if samplerate=44100 then sr=31
if samplerate=48000 then sr=28
v.setwritecolors(147,154) 											' print the info
position 184,4 : print "                                " 
if len(filename$)>33 then filename$="(...)"+right$(filename$,28)
