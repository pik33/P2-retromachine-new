...
var i,b:byte;
    bb:array[0..3] of byte absolute report;

begin
ConsoleWindowWrite(WindowHandle,inttostr(report[0])+' ');
ConsoleWindowWrite(WindowHandle,inttostr(report[1])+' ');
ConsoleWindowWrite(WindowHandle,inttostr(report[2])+' ');
ConsoleWindowWrite(WindowHandle,inttostr(report[3])+' ');
serialwrite(@bb,4,count)
{
for i:=0 to 3 do
  begin
  b:=report[i];
  SerialWrite(@b,1,Count);
  ConsoleWindowWrite(WindowHandle,inttostr(count)+' ');
  end;
...
if SerialOpen(115200,SERIAL_DATA_8BIT,SERIAL_STOP_1BIT,SERIAL_PARITY_NONE,SERIAL_FLOW_NONE,0,0) = ERROR_SUCCESS then
  ...
  while True do
    begin
    ConsoleWindowWriteLn(WindowHandle,'In loop,mousex='+inttostr(mousex));    sleep(100);
    ...
    if oldmx<>mousex then
       ...
       sendreport(report);
       ConsoleWindowWriteLn(WindowHandle,'Mouse X change');
    ...
    if key_scancode<>0 then
       ...
       sendreport(report);
         ...
         ConsoleWindowWriteLn(WindowHandle,'Key pressed');
         if key_charcode=13 then systemrestart(3);
        key_scancode:=0;
